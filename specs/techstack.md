...
**Retrieval modes for Sprint 6b+ evaluation**: `chat_messages.retrieval_mode` / session `model_settings.retrieval_mode` support multiple modes; boundaries must stay strict so evals stay meaningful:

- `rag` / `raw_transcript` (**Naive RAG baseline**) embeds and retrieves from `retrieval_embeddings` rows with `index_kind = raw_chunk` only (parsed document / transcript chunks). Must not use zettelkasten atomic notes, extracted entities, relationships, graph-context documents, Graphiti search, or graph traversal. `raw_transcript` is an alias used in North-history evals.
- `graph` (**Sprint 6 GraphRAG baseline**) uses Graphiti hybrid search over entities/relationships/episodes plus the graph-context grounding document. When a chat session scope includes `agent_id`, candidate relationship facts are intersected with documents owned by that agent before packing grounding docs.
- `hybrid` (**GraphRAG traversal**) uses deterministic structured handlers plus supporting graph evidence (same agent-aware filtering as `graph` when scoped).
- `zettelkasten_notes` retrieves from `retrieval_embeddings` with `index_kind = note_zettel` (vectors built from title+body after note creation).
- `amem_lite` retrieves from `index_kind = note_amem` (vectors rebuilt after A-MEM enrichment / dreaming touch so memory_context + keywords influence similarity).
