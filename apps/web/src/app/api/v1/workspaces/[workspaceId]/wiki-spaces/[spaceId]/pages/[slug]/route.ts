...
export async function GET(
  _req: Request,
  { params }: { params: { workspaceId: string; spaceId: string; slug: string } },
) {
  const { workspaceId, spaceId, slug } = params;
  ...
  const res = await pipelineFetch(
    `/internal/v1/workspaces/${encodeURIComponent(workspaceId)}/wiki-spaces/${encodeURIComponent(spaceId)}/pages/${encodeURIComponent(slug)}`,
