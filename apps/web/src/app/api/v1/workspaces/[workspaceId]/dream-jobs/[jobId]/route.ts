...
export async function GET(
  ...
  if (!uuidParam.safeParse(workspaceId).success || !uuidParam.safeParse(jobId).success) {
  ...
  const res = await pipelineFetch(
    `/internal/v1/workspaces/${encodeURIComponent(workspaceId)}/dream-jobs/${encodeURIComponent(jobId)}`,
